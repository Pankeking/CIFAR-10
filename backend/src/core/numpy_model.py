...
class NumpyModel:
    ...
    def create_model(
        self, number_samples: int, dataset_name: str, C_out: int = 32
    ) -> None:
        self.dataset_name = dataset_name
        print(
            f"Creating model with {self.dataset_name} dataset and {C_out} output channels"
        )
        ...
        layers = [
            Conv2DLayer(
                in_channels=C, out_channels=C_out, kernel_size=3, stride=1, padding=1
            ),
            ReLULayer(),
            Conv2DLayer(
                in_channels=C_out,
    ...
    def load(self, filepath: str) -> None:
        ...
        self.optimizer = Optimizer(
            optimizer_mode=data.get("optimizer", OptimizerMode.SGD)
        )
        ...
        print(
            f"Model loaded from {os.path.join(model_directory, filepath)} with dataset {self.dataset_name}"
        )
