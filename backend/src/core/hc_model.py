...
class HCModel(NumpyModel):
    ...
    def _train(self, epochs: int, metrics: bool = False):
        for epoch in range(epochs):
            ...
            gradient_hidden_linear_2 = (
                gradient_hidden_activation_2
                * hidden_activation_2
                * (1 - hidden_activation_2)
            )
            ...
            gradient_hidden_linear = (
                gradient_hidden_activation * hidden_activation * (1 - hidden_activation)
            )
            gradient_weights_1 = self.input_data_shape.T.dot(gradient_hidden_linear)

            new_weights.append(
                self.weights[0] - (self.learning_rate * gradient_weights_1)
            )
            new_weights.append(
                self.weights[1] - (self.learning_rate * gradient_weights_2)
            )
            new_weights.append(
                self.weights[2] - (self.learning_rate * gradient_weights_3)
            )
            ...
            if metrics and epoch % 100 == 0:
                print(f"Epoch {epoch}:")
                print_metrics(self.y_prediction, self.output_data_shape, self.loss)
